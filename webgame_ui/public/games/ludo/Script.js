document.addEventListener("DOMContentLoaded", () => {
	...
	let SQUARE_SIZE; // Kích thước 1 ô vuông (px) - phải khớp với CSS
	...
	function initializeBoardGraphics() {
		// trước hết đọc lại kích thước thực của board
		SQUARE_SIZE = boardElement.clientWidth / BOARD_SIZE_GRID; // 15 là số ô trên mỗi cạnh

		// Xóa các ô và quân cờ cũ trước khi vẽ lại
		boardElement
			.querySelectorAll(".square, .piece")
			.forEach((el) => el.remove());

		pathCoordinates.fill(undefined); // Reset mảng tọa độ
		Object.values(homePathCoordinates).forEach((arr) =>
			arr.fill(undefined)
		);
		...
		pathDef.forEach((pos, index) => {
			...
			square.style.top = `${pos.r * SQUARE_SIZE}px`;
			square.style.left = `${pos.c * SQUARE_SIZE}px`;
	...
	class Piece {
		constructor(player, id) {
			...
			// --- Nhúng SVG Chess Knight từ file và đổi màu theo player ---
			fetch("Chess-Knight--Streamline-Ultimate.svg")
				.then((res) => res.text())
				.then((svgText) => {
					...
					const fillColor = colorMap[this.player] || "black";

					// Gán màu fill và stroke
				...
				.catch((err) => console.error("Lỗi load SVG:", err));
		...
		resetPosition() {
			...
			this.position = -1;
			this.stepsTaken = 0;
			const stableInner = document
				.getElementById(`stable-${this.player}`)
				.querySelector(".stable-inner");
			if (stableInner) {
		...
		moveToPath(pathIndex) {
			...
			if (coords) {
				const pieceSize =
					parseInt(window.getComputedStyle(this.element).width) || 32; // Lấy kích thước thực tế từ CSS hoặc mặc định
				this.element.style.top = `${coords.top + (squareSize - pieceSize) / 2}px`;
				this.element.style.left = `${coords.left + (squareSize - pieceSize) / 2}px`;
			} else {
				console.error(
					`Invalid path index ${this.position} for piece ${this.id}`
				);
			}
		}
		...
		moveToHomePath(homeIndex) {
			...
			if (coords) {
				const pieceSize =
					parseInt(window.getComputedStyle(this.element).width) || 32;
				...
				console.error(
					`Invalid home path index ${homeIndex} for player ${this.player}`
				);
			}
		}
	...
	Piece.prototype.moveToHomePath = function (homeIndex) {
		this.state = "in_home_path";
		this.position = homeIndex;
		this.element.classList.remove("in-stable");
		this.element.style.position = "absolute";
		...
		// Tọa độ ô home đã lưu trong homePathCoordinates
		const coords = homePathCoordinates[this.player][homeIndex - 1];
		if (!coords) {
			return console.error(`Invalid home index ${homeIndex}`);
		}

		// Lấy kích thước ô và kích thước piece thực tế
		const squareSize = SQUARE_SIZE;
		const pieceSize = this.element.getBoundingClientRect().width;
	...
	function resetDiceFace() {
		document.querySelectorAll(".dice-face").forEach((g) => {
			g.style.display = "none";
		});

		const defaultFace =
			diceSvg.querySelector("#face-question") ||
			diceSvg.querySelector("#face-1");
		if (defaultFace) defaultFace.style.display = "block";
	}
	...
	function rollDice() {
		...
		requestAnimationFrame(() => {
			requestAnimationFrame(() => {
				diceSvg.style.animation = "dice-spin 0.6s ease-out";
			});
		});
		...
		// Ẩn hết các face
		// --- Hiệu ứng: liên tục hiện mặt random ---
		let shakeInterval = setInterval(() => {
			const tempValue = Math.floor(Math.random() * 6) + 1;
			document
				.querySelectorAll(".dice-face")
				.forEach((g) => (g.style.display = "none"));
			const tempFace = diceSvg.querySelector(`#face-${tempValue}`);
			if (tempFace) tempFace.style.display = "block";
		}, 100); // cứ 100ms thay mặt 1 lần

		// --- Sau khi xoay xong (~600ms), hiện mặt thật ---
		setTimeout(() => {
			clearInterval(shakeInterval); // dừng lắc
			...
			document
				.querySelectorAll(".dice-face")
				.forEach((g) => (g.style.display = "none"));
			const finalFace = diceSvg.querySelector(
				`#face-${gameState.diceValue}`
			);
			if (finalFace) finalFace.style.display = "block";

			rollButton.disabled = true;

			// console.log(
			// 	`${gameState.currentPlayer} rolled a ${gameState.diceValue}`
			// );
		...
		}, 600); // phải khớp với thời gian animation CSS
	}
	...
	function findValidMoves() {
		...
		for (const pieceId in gameState.pieces) {
			...
			if (piece.state === "stable") {
				...
				if (potentialStepsTaken < stepsToPreHome) {
					...
					for (let i = 1; i < dice; i++) {
						...
						if (
							getPieceAtBoardPosition(intermediatePos, "on_path")
						) {
							// Bất kỳ quân nào cũng chặn
							isPathBlocked = true;
					...
					if (homePathIndex <= HOME_PATH_LENGTH) {
						...
						const stepsOnPathBeforeHome =
							stepsToPreHome - 1 - piece.stepsTaken;
						...
						if (!isPathBlocked) {
							for (let h = 1; h < homePathIndex; h++) {
								// Kiểm tra các ô home path trung gian (1 đến index-1)
								if (
									getPieceAtBoardPosition(
										h,
										"in_home_path",
				...
				if (potentialHomePos <= HOME_PATH_LENGTH) {
					...
					for (let i = 1; i < dice; i++) {
						...
						if (
							intermediateHomePos < potentialHomePos &&
							intermediateHomePos < HOME_PATH_LENGTH
						) {
							if (
								getPieceAtBoardPosition(
									intermediateHomePos,
			...
			if (moveType !== "invalid") {
				...
				if (targetState === "on_path") {
					...
					if (occupyingPiece) {
						if (occupyingPiece.player === player) {
							...
							if (moveType === "stable_out")
								moveType = "stable_out_capture";
				...
				if (isTargetValid) {
					// Thêm vào validMoves nếu đường đi không bị chặn VÀ ô đích hợp lệ
		...
		// console.log(
		// 	`Valid moves for ${player} with dice ${dice} (blocking rule applied):`,
		// 	gameState.validMoves
		// );
	...
	function highlightValidPieces(highlight) {
		document
			.querySelectorAll(".piece.highlight")
	...
	function handlePieceClick(clickedPiece) {
		...
		const validMove = gameState.validMoves.find(
			(move) => move.pieceId === clickedPiece.id
		);
	...
	function updateTurnIndicator() {
		...
		// Gán class cho message-area
		messageArea.classList.remove("red", "blue", "green", "yellow");
		messageArea.classList.add(gameState.currentPlayer);
	}
	...
	function checkWinCondition(player) {
		...
		for (const pieceId in gameState.pieces) {
			if (
				gameState.pieces[pieceId].player === player &&
				gameState.pieces[pieceId].state === "finished"
			) {
				finishedCount++;
			}
		}
	...
	function aiChooseMove() {
		...
		if (difficulty === "easy") {
			...
			const scoredMoves = gameState.validMoves.map((move) => {
				...
				if (move.moveType === "stable_out")
					score += gameState.diceValue === 6 ? 210 : 200; // Ưu tiên ra bằng 6 hơn 1 chút
				...
				if (move.targetState === "on_path") {
					...
					score +=
						targetRelPos > currentRelPos
							? targetRelPos
							: targetRelPos + PATH_LENGTH; // Cộng điểm tiến lên
				...
				if (difficulty === "hard") {
					...
					bestMove =
						bestMoves[Math.floor(Math.random() * bestMoves.length)]
							.move; // Chọn ngẫu nhiên trong các nước tốt nhất
				}
			} else {
				bestMove =
					gameState.validMoves[
		...
		if (bestMove) {
			...
			console.error(
				"AI could not choose a best move!",
				gameState.validMoves
			);
	...
	window.addEventListener("resize", () => {
		if (gameState && gameState.turnState) {
			initializeBoardGraphics();
			// nếu đang chơi, cần đặt lại vị trí các piece (có thể lưu lại state và gọi moveToPath/moveToHomePath)
		}
	});
